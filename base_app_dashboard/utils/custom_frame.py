...
class ProcessedData:
    ...
    def get_gendered_data(self, gender, cutoff_year=None):
        ...
        if cutoff_year is None:
            ...
            self.gendered_data = self.gendered_data[
                self.gendered_data["Gender"] == gender
            ]
        else:
    ...
    def get_time_data(bare_frame):
        ...
        bare_frame["Swim+T1"] = round(
            bare_frame["Swim Minutes"] + bare_frame["T1 Minutes"], 2
        )
        bare_frame["Plus Bike"] = round(
            bare_frame["Swim+T1"] + bare_frame["Bike Minutes"], 2
        )
        bare_frame["Plus T2"] = round(
            bare_frame["Plus Bike"] + bare_frame["T2 Minutes"], 2
        )
        bare_frame["Total"] = round(
            bare_frame["Plus T2"] + bare_frame["Run Minutes"], 2
        )
