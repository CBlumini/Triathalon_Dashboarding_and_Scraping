...
# set some colors
colors = {"background": "#111111", "text": "#00ABE1"}
...
reduced2 = time_df[
    ["Name", "Swim Minutes", "Swim+T1", "Plus Bike", "Plus T2", "Total", "Gender Place"]
]
reduced2["Start"] = 0
...
app.layout = html.Div(
    [
        dbc.Container(
            [
                dbc.Row(
                    html.H1("Welcome to the Triathlon Data Analyzer"), justify="center"
                ),
                dbc.Row(
                    html.H6(
                        children="This app allows for plotting of certain Bay Area triathlons."
                    )
                ),
            ]
        ),
...
@app.callback(
    Output("pie-chart", "figure"), Input("names", "value"), Input("places", "value")
)
...
def update_figure_scat(places):
    ...
    dimensions = list(
        [
            ...
            dict(
                range=[reduced3["Swim Minutes"].min(), reduced3["Swim Minutes"].max()],
                label="Time After Swim",
                values=reduced3["Swim Minutes"],
            ),
            dict(
                range=[reduced3["Swim+T1"].min(), reduced3["Swim+T1"].max()],
                label="Time After First Transition",
                values=reduced3["Swim+T1"],
            ),
            dict(
                range=[reduced3["Plus Bike"].min(), reduced3["Plus Bike"].max()],
                label="Time After Bike",
                values=reduced3["Plus Bike"],
            ),
            dict(
                range=[reduced3["Plus T2"].min(), reduced3["Plus T2"].max()],
                label="Time After Second Transition",
                values=reduced3["Plus T2"],
            ),
            dict(
                range=[reduced3["Total"].min(), reduced3["Total"].max()],
                label="Total Time",
                values=reduced3["Total"],
            ),
            dict(
                range=[0, reduced3["Gender Place"].max()],
                tickvals=reduced3["Gender Place"],
                ticktext=reduced3["Name"],
                label="Competitor",
                values=reduced3["Gender Place"],
            ),
        ]
    )
    ...
    para_cor = go.Figure(
        data=go.Parcoords(
            line=dict(
                color=reduced3["Gender Place"],
                colorscale=[
                    [0.0, "rgba(255,0,0,0.1)"],
                    [0.2, "rgba(0,255,0,0.1)"],
                    [0.4, "rgba(0,0,255,0.1)"],
                    [0.6, "rgba(0,255,255,0.1)"],
                    [0.8, "rgba(255,0,255,0.1)"],
